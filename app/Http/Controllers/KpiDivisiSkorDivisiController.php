    ...
    public function index(Request $request)
    {
        ...
        if (is_null($bulan) || is_null($tahun)) {
            ...
            return view('kpi-divisi-skor-divisi.index', [
                'me'=>$me,'divisions'=>$divisions,'divisionsAll'=>$divisionsAll,
        ...
        foreach ($divisions as $div) {
            ...
            // Normalisasi bobot AHP di dalam tiap tipe
            $kpiWeightInType = [];
            foreach ($kpisByType as $t => $rowsKpi) {
                $sum = (float)$rowsKpi->sum(fn($k)=> (float)($k->bobot ?? 0));
                if ($sum > 0) {
                    foreach ($rowsKpi as $kpi) $kpiWeightInType[$t][$kpi->id] = ((float)$kpi->bobot) / $sum;
                } else {
                    $n = $rowsKpi->count();
                    if ($n > 0) { $eq = 1.0 / $n; foreach ($rowsKpi as $kpi) $kpiWeightInType[$t][$kpi->id] = $eq; }
                }
            }

            $userIds = User::where('division_id', $divisionId)->where('role','karyawan')->pluck('id')->all();

            // KUANTITATIF (rata-rata karyawan)
            $qHdr = KpiDivisiKuantitatifRealization::where('division_id',$divisionId)
                ->where('bulan',$bulan)->where('tahun',$tahun)
                ->where('status','approved')->whereIn('user_id', $userIds)->get()->keyBy('user_id');

            $qItems = $qHdr->isNotEmpty()
                ? KpiDivisiKuantitatifRealizationItem::whereIn('realization_id', $qHdr->pluck('id'))->get()->groupBy('realization_id')
                : [];
            $perUserQ = [];
            ...
            $kItems = $kHdr->isNotEmpty()
                ? KpiDivisiKualitatifRealizationItem::whereIn('realization_id', $kHdr->pluck('id'))->get()->groupBy('realization_id')
                : [];
            $perUserK = [];
            ...
            $rItems = $rHdr->isNotEmpty()
                ? KpiDivisiResponseRealizationItem::whereIn('realization_id', $rHdr->pluck('id'))->get()->groupBy('realization_id')
                : [];
            $perUserR = [];
            ...
            if ($kpisByType['persentase']->count() > 0) {
                ...
                $persReal = KpiDivisiPersentaseRealization::whereIn('kpi_divisi_id',$persIds)
                    ->where('division_id',$divisionId)->where('bulan',$bulan)->where('tahun',$tahun)
            ...
            $rows[$divisionId] = [
                'q'=>$scoreQ,'k'=>$scoreK,'r'=>$scoreR,'p'=>$scoreP,
                'total'=>$total
            ];
        }
